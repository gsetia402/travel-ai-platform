...
def list_documents(traveller_id: str, db: Session = Depends(get_db)):
    try:
...
def verify_document(document_id: str, request: VerifyRequest = VerifyRequest(), db: Session = Depends(get_db)):
    try:
...
def reject_document(document_id: str, request: VerifyRequest = VerifyRequest(), db: Session = Depends(get_db)):
    try:
...
def create_requirement(trip_id: str, request: RequirementCreateRequest, db: Session = Depends(get_db)):
    try:
...
def get_requirements(trip_id: str, db: Session = Depends(get_db)):
    try:
...
def document_summary(trip_id: str, db: Session = Depends(get_db)):
    try:
...
def traveller_readiness(traveller_id: str, db: Session = Depends(get_db)):
    try:
